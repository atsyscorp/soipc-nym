<!-- Ventana consulta operaciones ventanilla -->
<div align="center" style="width:100%; margin-top:25px;">
	<?=$msbloc=dBloc()?>
	<?php
	//---------------------------------------------------
	//Carga configuracion cambiaria
	$getExcSet = ExcSet($link);
	//---------------------------------------------------
	//Carga combo sucursales
	$strSQS = "SELECT Codigo_Sucursal FROM Sucursales";
	//Carga combo monedas
	$strSQM = "SELECT Moneda FROM XConf_Monedas WHERE Moneda <> '". $sExcSet[3] ."'";
	//-------------------------------------------------
	//Consulta de campos de tabla
	$strSQC = "SELECT * FROM Operacion_Ventanilla LIMIT 0, 1";
	$pc = mysqli_query($link, $strSQC) or die(mysqli_error($link));
	$ic = mysqli_num_fields($pc);
	?>
	<div align="center" style="width:440px; position:relative">
		<div id="dpup2" style="position:relative; top:0px; left:0px">
			<?=$msj=MsSucss('drod_1 dlin_5 bgcol_3', 'falert', 5, 436, 2, '', '', 1, '', 1, 'hidden')?>
		</div>
		<div class="drod_1 dlin_1 dsom_3 bgcol_2" align="left" style="width:100%">
			<div class="bgcol_6" style="overflow:hidden">
				<div style="float:left; font-weight:bold; margin-left:5px; margin-top:6px" class="fwhite">Reporte movimientos ventanilla</div>
				<div style="float:right; margin:3px" class="dmMain bgcol_1" onclick="hidcap('dConsCri')"><img src="images/close.png" style="height:20px; width:auto" /></div>
			</div>
			<div style="margin:0px">
				<div style="width:100%">
					<div id="dCriConsulta" class="bttabsel_1 dsition_4" style="width:35%" onClick="Sel_TabC(this)">Criterios de consulta</div>
					<div id="dAvanzadas" class="bttab_1 dsition_4" style="width:35%" onClick="Sel_TabC(this)">Opciones avanzadas</div>
					<div style="width:30%; border-bottom:#BEBEBE solid 1px; float:left; height:35px"></div>
					<div style="clear:both"></div>
				</div>
				<div style="margin:15px 8px">
					<!-- Criterios de consulta -->
					<div id="dCriConsultaT">
						<table style="width:100%" cellpadding="0" cellspacing="0">
							<tr class="trtxco">
								<td style="width:40%; text-align:left" class="fcont">Tipo de operación:</td>
								<td align="right" style="width:60%">
									<select name="Tipo_Operacion" id="tx1" style="width:100%;" class="txboxo" onchange="Sel_MovOps()">
										<option value=""></option>
										<option value="Operacion_Ventanilla||">COMPRAS Y VENTAS</option>
										<option value="Operacion_Ventanilla|COMPRA DE DIVISAS|Tipo_Operacion">COMPRA DE DIVISAS</option>
										<option value="Operacion_Ventanilla|VENTA DE DIVISAS|Tipo_Operacion">VENTA DE DIVISAS</option>
										<option value="Traslados_Ventanilla||">INGRESOS Y EGRESOS</option>
										<option value="Traslados_Ventanilla|INGRESO|Tipo_Movimiento">INGRESOS</option>
										<option value="Traslados_Ventanilla|EGRESO|Tipo_Movimiento">EGRESOS</option>
										<option value="Egresos_Ventanilla|PAGOS DESDE VENTANILLA|Tipo_Movimiento">PAGOS DESDE VENTANILLA</option>
										<option value="Cierres_Ventanilla||">CIERRES VENTANILLA</option>
									</select>
								</td>
							</tr>
							<tr class="trtxco">
								<td style="text-align:left" class="fcont">Fecha inicial:</td>
								<td align="right"><input type="date" name="Fecha" id="tx2" maxlength="30" class="txboxo" style="width:98%;" value="" /></td>
							</tr>						
							<tr class="trtxco">
								<td style="text-align:left" class="fcont">Fecha final:</td>
								<td align="right"><input type="date" name="Fecha" id="tx3" maxlength="30" class="txboxo" style="width:98%;" value="" /></td>
							</tr>						
							<tr class="trtxco">
								<td style="text-align:left" class="fcont">Seleccione sucursal:</td>
								<td align="right">
									<select name="Sucursal" id="tx4" style="width:100%;" class="txboxo">
										<option value=""></option>
										<option value="TODAS">TODAS</option>
										<?=$cbLoad=LoadConfTab($link, $strSQS)?>
									</select>
								</td>
							</tr>						
							<tr class="trtxco">
								<td style="text-align:left" class="fcont">Moneda (Divisa):</td>
								<td align="right">
									<select name="Moneda" id="tx5" style="width:100%;" class="txboxo">
										<option value=""></option>
										<option value="TODAS">TODAS</option>
										<?=$cbLoad=LoadConfTab($link, $strSQM)?>
									</select>
								</td>
							</tr>						
							<tr class="trtxco">
								<td style="text-align:left" class="fcont">Medio de pago:</td>
								<td align="right">
									<select name="Medio_Pago" id="tx6" style="width:100%;" class="txboxo">
										<option value=""></option>
										<option value="TODOS">TODOS</option>
										<option value="EFECTIVO">EFECTIVO</option>
										<option value="BANCOS">BANCOS</option>
									</select>
								</td>
							</tr>						
							<tr class="trtxco">
								<td style="text-align:left" class="fcont">Mostrar anulados:</td>
								<td align="right">
									<select name="Medio_Pago" id="tx7" style="width:100%;" class="txboxo">
										<option value=""></option>
										<option value="SI">SI</option>
										<option value="NO">NO</option>
									</select>
								</td>
							</tr>						
							<tr class="trtxco">
								<td style="text-align:left" class="fcont">Ordenar por:</td>
								<td align="right">
									<select name="cbOrder" id="tx8" style="width:100%;" class="txbox" onChange="Sel_OrderC()">
										<option value=""></option>
									</select>
								</td>
							</tr>						
							<tr class="trtxco">
								<td style="text-align:left" class="fcont">Tipo ordenación:</td>
								<td align="right">
									<select name="cbOrderType" id="tx9" style="width:100%;" class="txbox">
										<option value=""></option>
										<option value="asc">Ascendente</option>
										<option value="desc">Descendente</option>
									</select>
								</td>
							</tr>						
						</table>
					</div>
					<!-- Opciones avanzadas -->
					<div id="dAvanzadasT" style="display:none">
						<div style="width:98%; margin:auto">
							<div>
								<div class="fgreen">Seleccione los campos en reporte</div>
								<div style="margin-top:7px; padding:8px 0; max-height:250px; overflow:auto" class="dlin_1 bgcol_1">							
									<div id="dchfields" style="width:96%; margin:auto">
									</div>
								</div>
							</div>						
							<div style="margin-top:15px">
								<div class="fgreen">Seleccione tabla para relacionar</div>
								<div style="margin-top:5px">
									<select name="Relaciona" id="Relaciona" style="width:100%;" class="txbox">
										<option value=""></option>
										<option value="Sucursales|Codigo_Sucursal|Sucursal">Sucursales - Código Sucursal</option>
										<option value="Clientes|Identificacion|Documento_Beneficiario">Clientes - Documento beneficiario</option>
										<option value="Clientes|Identificacion|Documento_Declarante">Clientes - Documento declarante</option>
									</select>
								</div>
							</div>						
						</div>		
					</div>
				</div>
				<div style="margin:10px 8px">
					<div style="float:left; margin-right:10px"><input name="btacceptgen" id="btacceptgen" type="button" value="Generar reporte" class="btcont" style="width:120px;" onclick="RepMovVenta()" /></div>
					<div style="float:left;"><input name="btexitgen" id="btexitgen" type="button" value="Salir" class="btcont" style="width:88px;" onclick="hidcap('dConsCri')" /></div>
					<div style="clear:both"></div>
				</div> 
			</div>
		</div>
	</div>
</div>
