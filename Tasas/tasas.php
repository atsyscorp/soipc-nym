...
function TasaBox($sBack, $sHei, $sCur, $sBuy, $sSell)
{
	$sfun = '<div style="width:100%" class="'. $sBack. '">
					<div style="width:34%; float:left; text-align:left">
						...
						<div style="float:left; width:38.5%; text-align:left; margin-left:3%; margin-top:0%" class="txgen_1 txblack"><span style="font-size:7.5vw"><b>'.$sCur.'</b></span></div>
						<div style="clear:both"></div>
					</div>
					<div style="width:33%; float:left; text-align:center; margin-top:0%" class="txgen_1 txblack"><span style="font-size:7.5vw"><b>'.$sBuy.'</b></span></span>
					</div>						
					<div style="width:33%; float:left; text-align:center; margin-top:0%" class="txgen_1 txblack"><span style="font-size:7.5vw"><b>'.$sSell.'</b></span></span>
					</div>						
					<div style="clear:both"></div>
				</div>';
	//----------------------------------------------------
...
function ShowTasas()
{
	...
	while($nu=mysqli_fetch_array($u)){
		if($sSuc != 'G20'){
			echo TasaBox('bgcol_3 dlin_5', '', $nu[2], FPrice($nu[5]), FPrice($nu[7]));
		} else {
			echo TasaBox('bgcol_3 dlin_5', '', $nu[2], FPrice($nu[5]), 'N/A');
		}
	}
	//Euro
	...
	while($ne=mysqli_fetch_array($e)){
		if($sSuc != 'G20'){
			echo TasaBox('bgcol_3 dlin_5', '', $ne[2], FPrice($ne[5]), FPrice($ne[7]));
		} else {
			echo TasaBox('bgcol_3 dlin_5', '', $ne[2], FPrice($ne[5]), 'N/A');
		}
	}
	//El resto de monedas
	$strSQLG = "SELECT * FROM Tasas Where Sucursal = '". $sSuc. "' AND Estacion='01' AND Moneda<>'EUR' AND Moneda<>'USD' Order by Moneda";
	$g=mysqli_query($link, $strSQLG) or die(mysqli_error($link));
	$k = 0;
	while($ng=mysqli_fetch_array($g)){
		if($sSuc != 'G20'){
			echo TasaBox('bgcol_3 dlin_5', '', $ng[2], FPrice($ng[5]), FPrice($ng[7]));
		} else {
			echo TasaBox('bgcol_3 dlin_5', '', $ng[2], FPrice($ng[5]), 'N/A');
		}
	}
}
?>