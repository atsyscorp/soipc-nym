// JavaScript Document
//=============================================================
//FUNCIONES PARA VENTANA FRREPOPSCLOSE.PHP
//=============================================================
//Carga de ventana
...
function cmAccept_Clic()
{
	...
	var cbSuc = document.getElementById('tx1');
	var cbEst = document.getElementById('tx2');
	var strSUC = " AND Sucursal = '" + cbSuc.value + "' AND Estacion = '" + cbEst.value + "'";
	var strFEC = sIni + ".|." + sFin;
	var lstRep = document.getElementById('tbTCList');
	var sTbIni = lstRep.innerHTML; 
	lstRep.innerHTML = sTbIni +  GenConret_1('ajax/frRepOpsClose', 'RepValida', strSUC, strFEC, false);
	hidcap('dbloc');
	hidcap('dWait');
	//------------------------------------
...
function cmExport_Click(sCompa)
{
	...
	var sFin = cbYear1.value + "-" + cbMonth1.value + "-" + cbDay1.value;
	viscap('dWait');
	ExportToHtml('Validación cierres', sCompa, 'Validación cierres<br />Desde: ' + sIni + ' Hasta: ' + sFin, 'dRepExpo');
	hidcap('dWait');
}
