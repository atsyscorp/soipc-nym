...
function cbCurr_Change(ssuc, scaja)
{
	...
	//se borra la mia el 23012024 por que afectaba cuando uno compra otras monedas por x estacion y las quiere luego vender en otra estacion, se concluyo que para dar solucion provisional es crear una estacion 4	var strSQLB = "Select Precio_Base From Tasas Where Moneda='" + txCur.value + "' AND Sucursal='" + ssuc + "' AND Estacion = '" + scaja + "'";
		...
		if(isNaN(vVV0) == false && isNaN(vCS0) == false && vVV0 < vCS0){
			...
			//Traslados Entradas
			var strSQLTE0 = "SELECT SUM(Cantidad) FROM Traslados_Ventanilla WHERE Codigo_Movimiento='142' AND Fecha='" + dfechav.value + "' AND Moneda='" + txCur.value + "' AND Estado='ACTIVO' AND Sucursal='" + ssuc + "' AND Estacion='01' AND Origen_Destino NOT LIKE '%CAJA%'";
			var vDE0 = GenConretField('General', 'Gen_Find_Field', strSQLTE0, false);
			var strSQLTE1 = "SELECT SUM(Valor) FROM Traslados_Ventanilla WHERE Codigo_Movimiento='142' AND Fecha='" + dfechav.value + "' AND Moneda='" + txCur.value + "' AND Estado='ACTIVO' AND Sucursal='" + ssuc + "' AND Estacion='01' AND Origen_Destino NOT LIKE '%CAJA%'";
...
function cmCOOCalc_Click()
{
	...
	//Valor descontado
	document.getElementById("tx48").value = (parseFloat(DelMilsepa(document.getElementById("tx47").value)) - parseFloat(DelMilsepa(document.getElementById("tx41").value))).toFixed(2);
	//----------------------------------------------------------
	//Si entra desde cliente activa aceptar
	if(document.getElementById("tx20").value != '')
	{
...
{
	var ssuc = document.getElementById("tx4").value;
	var smon = document.getElementById("tx34").value;
	var cCant = GenConretField('General','Gen_Find_Field',"SELECT Cantidad FROM Stock_Valorizado WHERE Sucursal='" + ssuc + "' AND Moneda='" + smon + "'",false);
	if(cCant != '' && parseFloat(DelMilsepa(document.getElementById("tx37").value)) > parseFloat(cCant))
	{
		viscap('dbloc');
		dmsshowc("drod_1 dlin_5 bgcol_3", "falert", "hidden", "hidcap('dMsj" + 1 + "'); hidcap('dbloc')", "", "No hay suficiente stock para realizar esta transacción", 1);
		return false;
	} else {
		//Validaciones
		//Fecha de cierre
...
function Com_Print(iPrint)
{
	...
	{
        //disbtn('btprint');
		var iCPrint = GenConretField('General', 'Gen_Find_Field', "Select Copias_Venta From Configuracion_Contable Where Sucursal ='" + document.getElementById('tx4').value + "' AND Tipo_Resolucion='venta'", false);
		console.log('iCPrint', iCPrint);
